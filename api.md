...
* **类型** `object` 只读
* **说明**
返回当前环境画布信息，默认是root所在画布的坐标和尺寸还有相对根节点。当配合cacheAsBitmap生成离屏缓存时，返回离屏画布信息。
* **示例**
```jsx
class $ extends karas.Geom {
  render(renderMode, ctx, dx, dy) {
    ...
    // { x: 100, top: 100, width: 360, height: 360, node: Root }
  }
}
let root = karas.render(
...
* **参数**
  ...
  * cb `Function`
    更新且刷新后的回调。
* **说明**  
同步更新格式化好的样式。它要求传入的样式key和value均是格式化的，以提升性能，并且不会覆盖静态样式，即只存在于currentStyle中。
* **示例**
```jsx
let root = karas.render(
  <canvas>
