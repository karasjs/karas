(function (global, factory) {
    ...
    _createClass(Component, [{
      ...
      value: function __init() {
        ...
        if (sr instanceof Node) {
          ...
          sr = new Text(s); // 文字视作为父节点的直接文字子节点
          ...
          if (GRADIENT_TYPE.hasOwnProperty(st.k)) {
            ...
            if (k === 'points' || k === 'controls') {
              for (var _i6 = 0, _len3 = Math.min(_st.length, v.length); _i6 < _len3; _i6++) {
                if (isNil$4(_st[_i6]) || !_st[_i6].length) {
                  continue;
                }

                for (var j = 0, len2 = Math.min(_st[_i6].length, v[_i6].length); j < len2; j++) {
                  if (!isNil$4(_st[_i6][j]) && !isNil$4(v[_i6][j])) {
  ...
  var Xom = /*#__PURE__*/function (_Node) {
    ...
    function Xom(tagName) {
      ...
      _this.__children = flatten(_assertThisInitialized(_this), children);
      _this.__matrix = null;
      _this.__matrixEvent = null; // 考虑继承的matrix，直接计算事件触发
    ...
    _createClass(Xom, [{
      ...
      value: function __repaint(isRoot) {
        repaint$2(this, isRoot); // 即便自己不需要计算，但children还要继续递归检查
