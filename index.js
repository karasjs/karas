  ...
  var KawaseBlurFilter = /*#__PURE__*/function () {
    ...
    _createClass(KawaseBlurFilter, [{
      ...
      value: function draw(image, uOffsetArray, clear) {
        var uOffset = this.textureLocations.uOffset;
        var gl = this.gl;
  ...
  function unify(frames, target) {
    ...
    frames.forEach(function (item) {
      ...
      keys.forEach(function (k) {
        if (!style.hasOwnProperty(k) || isNil$3(style[k])) {
          if (repaint.GEOM.hasOwnProperty(k)) {
    ...
    _createClass(Xom, [{
      ...
      value: function updateStyle(style, cb) {
        var root = this.root,
            ...
            __cacheStyle = this.__cacheStyle,
            props = this.props,
            __currentProps = this.__currentProps,
            _this$__cacheProps = this.__cacheProps,
            __cacheProps = _this$__cacheProps === void 0 ? {} : _this$__cacheProps;

        if (root) {
          var lv = level.REPAINT;
          var hasUpdate, hasZIndex;
          var p;
    ...
    if (target && vars) {
      Object.keys(target).forEach(function (k) {
        if (k.indexOf('var-') === 0) {
          ...
          var k2 = k.slice(4); // 有id且变量里面传入了替换的值，值可为空，因为某些情况下空为自动

          if (v.id && vars.hasOwnProperty(v.id)) {
            var value = vars[v.id]; // 如果有.则特殊处理子属性
