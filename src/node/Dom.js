...
class Dom extends Xom {
  ...
  __calBasis(isDirectionRow, data) {
    ...
    if(isFixed) {
      if(flexBasis[1] === PX) {
        ...
        fixedSize = flexBasis[0] * this.root.computedStyle[FONT_SIZE];
      }
    ...
    // 已声明主轴尺寸的，当basis是auto时为main值
      ...
      if(main[1] === PX) {
        ...
        fixedSize = main[0] * this.root.computedStyle[FONT_SIZE];
      }
      else if(main[1] === VW) {
  ...
  __layoutFlex(data, isVirtual) {
    ...
    __flexLine.forEach(item => {
      ...
      let [x1, y1, maxCross] = this.__layoutFlexLine(clone, isDirectionRow, isVirtual, containerSize,
   ...
   * 先收集basis和假设主尺寸
   */
  __layoutFlexLine(data, isDirectionRow, isVirtual, containerSize,
                   fixedWidth, fixedHeight, lineClamp, lineClampCount,
                   lineHeight, computedStyle, justifyContent, alignItems,
  ...
  __layoutAbs(container, data, target) {
    ...
    absChildren.forEach(item => {
      ...
      if(w2 === undefined) {
        // 对比需去除abs元素本身的mbp
        let computedStyle = item.computedStyle;
        widthLimit -= computedStyle[MARGIN_LEFT] + computedStyle[MARGIN_RIGHT]
          + computedStyle[PADDING_LEFT] + computedStyle[PADDING_RIGHT]
          + computedStyle[BORDER_LEFT_WIDTH] + computedStyle[BORDER_RIGHT_WIDTH];
        let wa = item.__adjustWidth(widthLimit);
        // 对比需去除abs元素本身的mbp
        // let computedStyle = item.computedStyle;
        // wa += computedStyle[MARGIN_LEFT] + computedStyle[MARGIN_RIGHT]
        //   + computedStyle[PADDING_LEFT] + computedStyle[PADDING_RIGHT]
  ...
  __adjustWidth(widthLimit) {
    ...
    if(display === 'flex') {
      ...
      for(let i = 0, len = flowChildren.length; i < len; i++) {
        let item = flowChildren[i];
        let w = item.__adjustWidth(widthLimit);
        if(isRow) {
          count += w;
        }
        else {
